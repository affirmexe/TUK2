...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $email && $password && $confirm_password) {
        ...
        if ($stmt->rowCount() > 0) {
            $_SESSION['error'] = "A user with that email or name already exists.";
            header("Location: register.php");
            exit;
        }
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        // Insert with empty defaults for address and no_telp; update later via user settings.
        $stmt = $pdo->prepare("INSERT INTO users (name, address, email, no_telp, password, created_at) VALUES (?, ?, ?, ?, ?, NOW())");
